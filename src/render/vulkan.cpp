...
static vulkan::host_data_t* vulkan_data = nullptr;
...
vulkan::host_data_t* vulkan::api_handle(sigil::vmnode_t *node) {
    //if (node != vulkan_node) return nullptr;
    return vulkan_data;
}
...
sigil::status_t setup_vulkan_descriptor_pool() {
    ...
    err = vkCreateDescriptorPool(vulkan_data->vk_dev, &pool_info, 
...
sigil::status_t initialize_vulkan_instance() {
    ...
    sigil::vulkan::check_result(err);
    printf("SigilVM: Created managed Vulkan instance\n");
    return sigil::VM_OK;
}
...
sigil::status_t sigil::vulkan::initialize(sigil::vmnode_t *vmsr) {
    ...
    if (!platform) return sigil::VM_INVALID_ROOT;

    vulkan_node = platform->spawn_subnode("vulkan");
    if (!vulkan_node) return sigil::VM_FAILED_ALLOC;

    vulkan_data = new host_data_t();
    if (!vulkan_data) return sigil::VM_FAILED_ALLOC;

    vulkan_node->set_data(vulkan_data, cleanup_vulkan_data);

    sigil::status_t err;
    err = initialize_vulkan_instance();
    err = initialize_graphics_devices();
    err = initialize_vulkan_queues();
